...
def voting_users(request, vote_id):
    vote_users = Choose_option.objects.filter(option_text__vote_id=vote_id).select_related('user')
    user_list = [{'username': user.user.username, 'image_profile': user.user.profile.get_image_url()} for user in vote_users]
    return JsonResponse({'users': user_list})

@login_required
def profile(request, user_id):
    user = User.objects.get(pk=user_id)
    user_votes = Create_vote.objects.filter(user=user)
    profile= Profile.objects.get(user=request.user)
    num_votes = user_votes.count() 
...
def edit_profile(request, user_id):
    ...
    return render(request, "wishes/edit_profile.html", {
        "user": user,
        "profile": profile
    })
...
def new_survey(request, user_id):
    ...
    if request.method == 'POST':
        ...
        closing_vote = request.POST.get('closingVote')  
...
def more_votes(request, vote_id):
    ...
    if vote.status == 'closed':
        ...
        if len(top_vote_options) > 1 or max_votes == 0:
            ...
            top_vote_options[0].option_text += " \n  🏆WINNER!"
